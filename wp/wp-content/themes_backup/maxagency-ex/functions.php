...
function maxagency_setup() {
	
	//Register a theme image size.
	add_theme_support( 'post-thumbnails' );
	add_image_size( 'maxagency-large-thumb', 830 );
	add_image_size( 'maxagency-medium-thumb', 550, 400, true );
	add_image_size( 'maxagency-small-thumb', 230 );
	
	//Register nav menu used in theme.
	register_nav_menus( array(
		'primary' 	=> __( 'Primary Menu', 'maxagency-ex' )
	) );
...
add_action( 'after_setup_theme', 'maxagency_setup' );


/*--- Blog layout ---*/
function maxagency_blog_layout() {
...
function maxagency_posted_on() {
	$layout = maxagency_blog_layout();
...
function maxagency_excerpt_more($more) {
	global $post;
	$layout = maxagency_blog_layout();
	return ' ....&nbsp;&nbsp;<a class=" '.$layout.'" href="'. get_permalink($post->ID) . '">' . __('Read More', 'maxagency-ex') . '</a>';
}
add_filter('excerpt_more', 'maxagency_excerpt_more');


/*--- Enqueue scripts and styles ---*/
function maxagency_scripts() {
	...
	wp_enqueue_script( 'maxagency-common', get_template_directory_uri() . '/js/common.js', array('jquery'), '20180213', true );
	...
	wp_enqueue_style( 'maxagency-font-awesome', get_template_directory_uri() . '/fonts/font-awesome.min.css' );
...
add_action( 'wp_enqueue_scripts', 'maxagency_scripts' );

// Enqueue Bootstrap
function maxagency_enqueue_bootstrap() {
	wp_enqueue_style( 'maxagency-bootstrap', get_template_directory_uri() . '/css/bootstrap/bootstrap.min.css', array(), true );
}
add_action( 'wp_enqueue_scripts', 'maxagency_enqueue_bootstrap', 9 );



/*--- Register widget area ---*/
add_action( 'widgets_init', 'maxagency_widgets_init' );
function maxagency_widgets_init() {
    register_sidebar( array(
        'name' => __( 'Main Sidebar', 'maxagency-ex' ),
        'id' => 'sidebar-1',
        'description' => __( 'Widgets in this area will be shown on all posts and pages.', 'maxagency-ex' ),
	...
	for ($i=1; $i<=3; $i++){
		register_sidebar( array(
			'name' => __( 'Footer-widget-area-', 'maxagency-ex' ) . $i,
			'id' => 'footer-widget-area' . $i,
			'description' => __( 'Widgets in this area will be shown on footer.', 'maxagency-ex' ),
			'before_widget' => '<li id="%1$s" class="widget %2$s">',
			'after_widget'  => '</li>',
			'before_title'  => '<h3 class="widgettitle">',
			'after_title'   => '</h3>',
...
function maxagency_edit_link() {
	edit_post_link(
		sprintf( __( '<i class="fa fa-edit"></i>Edit<span class="screen-reader-text"> "%s"</span>', 'maxagency-ex' ), get_the_title() ),
...
function maxagency_get_breadcrumb() {
    echo '<a href="'.home_url().'" rel="nofollow">'.__('Home', 'maxagency-ex').'</a>';
    if ( is_category() || is_single() ) {
        ...
        echo __("&nbsp;&nbsp;&#47;&nbsp;&nbsp;Search Results for... ", "agency");
        echo '"<em>';
...
function maxagency_recommended_plugin() {
...
add_action( 'tgmpa_register', 'maxagency_recommended_plugin' );
...
add_filter( 'pre_set_site_transient_update_themes', 'maxagency_check_for_update' );
function maxagency_check_for_update( $checked_data ){
...
popularfx_maybe_promo([
	'after' => 1,// In days
	'interval' => 10,// In days
	'pro_url' => 'https://popularfx.com/themes/wordpress/corporate/Maxagency_Pro',
	'rating' => 'https://popularfx.com/themes/wordpress/corporate/Maxagency-Ex',
